...
def train_model(model, training_data, testing_data, lr, epochs=1000, acc_interval=100, output=True):
    if output:
    ...
    for epoch in range(epochs):
        ...
        for batch, (train_data, train_labels) in enumerate(training_data):
            ...
            loss = criterion(output_labels.permute(0, 2, 1), train_labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            if (epoch * len(training_data) + batch + 1) % acc_interval % acc_interval == 0:
                acc = test_model(model, testing_data, False)
    if output:
...
def point_model_grid_search(model_data, range_nodes, batch_size, learning_rate, output=False):
    ...
    for num_nodes_in_hl in range(nodes_min, nodes_max, nodes_step):
        ...
        train_model(model, training_set, testing_set, learning_rate, output=output)

        final_acc = test_model(model, testing_set, output_updates=output)
...
def main():
    ...
    point_model_grid_search(model_data, num_nodes_in_hl, batch_size, learning_rate, output=True)
